 ...
 */
import { nBodyVisualizer } from './nBodyVisualizer'
const nBodyWasmUrl = 'nBodyWasm.wasm'
...
export class nBodySimulator {
  ...
  visualizations: nBodyVisualizer[] = []

  arrBodies: any[] = []
  arrForces: number[] = []
  worker: Worker
  forcesResolve: (stuff: unknown) => void = () => {}
  forcesReject: (error: unknown) => void = () => {}

  // Constructor below sets up web worker
  ...
  applyForces() {
    this.objBodies.forEach((body, i) => {
      ...
      // Convert to velocity.  We could remove mass in nBodyWasm.ts and just send velocities, but I'm moving this project to the done pile.
      body.vX = body.vX + body.forceX / body.mass
   ...
   */
  addVisualization(vis: nBodyVisualizer) {
  ...
  constructor() {
    ...
    this.worker = new Worker('nBodyWorker.js')
    ...
    WebAssembly.compileStreaming(fetch(nBodyWasmUrl))
